...
///
/// ``Client`` is the main object that you interact with to communicate with the API, and is responsible for making the underlying HTTP requests for each API operation and handling the resulting responses. It implements the ``VenariAIAPI`` protocol to support the available API operations.
///
/// ### API calls
///
/// Each API endpoint (or *operation*) is exposed as a method. For instance, the 'ProductData' operation is exposed as:
///
/// ```swift
...
///
/// The ``Response/data`` property defined by each API operation is an enum, and you can `switch` on this value to extract the corresponding value from the response. For example, the 'ProductData' operation returns a ``Operations/ProductDataResponse`` value:
///
/// ```swift
/// let response: Response<Operations.ProductDataResponse> = ... // Response from API call
/// switch response.data {
/// case .productWithUUID(let productWithUUID):
///     // Handle response
...
public final class Client {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            _makeRequest(with: server, configureRequest: configureRequest) { result in
                do {
                    ...
                    switch error {
                    case .internal:
                        continuation.resume(throwing: VenariAIError.internalError(error: nil))
                    case .missingDefaultServer(let serverType):
                        continuation.resume(throwing: VenariAIError.failedToConstructRequestURL(.missingDefaultServer(serverType: serverType)))
                    case .invalidServerIndex(let serverType, let index):
                        continuation.resume(throwing: VenariAIError.failedToConstructRequestURL(.invalidServerIndex(serverType: serverType, index: index)))
                    case .missingServerParameterSubstitutionKey(let key, let serverString):
                        continuation.resume(throwing: VenariAIError.failedToSerializeRequestParameters(.missingServerParameterSubstitutionKey(key, serverString: serverString)))
                    }
                    ...
                    case .invalidURL(let urlString):
                        continuation.resume(throwing: VenariAIError.failedToConstructRequestURL(.invalidRequestURL(string: urlString)))
                    case .missingPathParameterSubstitutionKey(let key, let path):
                        continuation.resume(throwing: VenariAIError.failedToSerializeRequestParameters(.missingPathParameterSubstitutionKey(key, path: path)))
                    }
                } catch let error as SerializationError {
                    switch error {
                    case .failedToSerializeData:
                        continuation.resume(throwing: VenariAIError.failedToSerializeRequestParameters(.failedToSerializeData))
                    case .missingRequiredRequestBody:
                        continuation.resume(throwing: VenariAIError.failedToConstructRequest(.missingRequiredRequestBody))
                    case .invalidSerializationParameter(let type, let format):
                        continuation.resume(throwing: VenariAIError.failedToSerializeRequestParameters(.invalidSerializationParameter(type: type, format: format)))
                    }
                } catch let error as ResponseHandlerError {
                    switch error {
                    case .failedToDecodeResponse:
                        continuation.resume(throwing: VenariAIError.failedToHandleResponse(.failedToDecodeResponse))
                    case .failedToDecodeJSON(let error):
                        continuation.resume(throwing: VenariAIError.failedToHandleResponse(.failedToDeserializeJSON(error)))
                    }
                } catch let error as VenariAIError {
                    continuation.resume(throwing: error)
                } catch {
                    continuation.resume(throwing: VenariAIError.internalError(error: error))
                }
            }
        }
    }
    ...
    private func baseURL(serverOverride: Server? = nil) throws -> URL {
        ...
        guard let url = URL(string: server.urlString) else {
            throw VenariAIError.failedToConstructRequestURL(.invalidServerURL(string: server.urlString))
        }
        return url
    }
    ...
    private func makeDataRequest(with urlRequest: URLRequest, completion: @escaping (Result<APIResponse, Swift.Error>) -> Void) {
        let task = _session.dataTask(with: urlRequest) { data, response, error in
            if let error {
                completion(.failure(VenariAIError.failedToMakeNetworkRequest(error: error)))
            } else if let httpResponse = response as? HTTPURLResponse {
                completion(.success(APIResponse(httpResponse: httpResponse, data: data)))
            } else {
                completion(.failure(VenariAIError.internalError(error: nil)))
            }
        }
        task.resume()
    }
